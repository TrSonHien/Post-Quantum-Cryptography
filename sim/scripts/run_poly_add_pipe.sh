...
iverilog -g2012 -Wall -I "$ROOT_DIR/rtl/common" -s tb_poly_add_pipe -o "$OUT/tb_poly_add_pipe.vvp" \
 ...
 "$ROOT_DIR/tb/block/tb_poly_binary_pipe.v" "$ROOT_DIR/tb/block/tb_poly_add_pipe.v"
timeout 30s vvp "$OUT/tb_poly_add_pipe.vvp" +VECTOR_FILE="$OUT/poly_add.mem" "${DEBUG_ARGS[@]}" | tee "${ROOT_DIR}/sim/logs/poly_add_pipe.log"
grep -q 'PASS tb_poly_binary_pipe' "${ROOT_DIR}/sim/logs/poly_add_pipe.log"
