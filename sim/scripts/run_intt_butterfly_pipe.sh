...
SIM_OUT="${OUT_DIR}/tb_intt_butterfly_pipe.vvp"
LOG_FILE="${LOG_DIR}/intt_butterfly_pipe.log"
...
iverilog -g2012 -Wall \
    ...
    "${ROOT_DIR}/tb/unit/tb_intt_butterfly_pipe.v"

vvp "${SIM_OUT}" | tee "${LOG_FILE}"

grep -q "PASS tb_intt_butterfly_pipe" "${LOG_FILE}"
