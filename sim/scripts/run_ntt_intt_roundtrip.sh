...
SIM_OUT="${OUT_DIR}/tb_ntt_intt_roundtrip.out"
LOG_FILE="${LOG_DIR}/ntt_intt_roundtrip_sim.log"
...
iverilog -g2012 -Wall \
    ...
    rtl/ntt/ntt_core.v \
    rtl/ntt/intt_core.v \
    tb/block/tb_ntt_intt_roundtrip.v 2>&1 | tee "${LOG_FILE}"

vvp "${SIM_OUT}" 2>&1 | tee -a "${LOG_FILE}"

grep -q "PASS tb_ntt_intt_roundtrip" "${LOG_FILE}"
