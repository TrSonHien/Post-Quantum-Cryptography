...
iverilog -g2012 -Wall -I "$ROOT/rtl/common" -s tb_poly_ntt_pipe -o "$OUT/tb_poly_ntt_pipe.vvp" \
 ...
 "$ROOT/tb/block/tb_poly_transform_pipe.v" "$ROOT/tb/block/tb_poly_ntt_pipe.v"
timeout 45s vvp "$OUT/tb_poly_ntt_pipe.vvp" +VECTOR_FILE="$OUT/poly_ntt.mem" "${DEBUG_ARGS[@]}"|tee "$ROOT/sim/logs/poly_ntt_pipe.log"
grep -q 'PASS tb_poly_transform_pipe' "$ROOT/sim/logs/poly_ntt_pipe.log"
