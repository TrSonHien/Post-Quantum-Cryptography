...
SIM_OUT="${OUT_DIR}/tb_butterfly_unit.vvp"
LOG_FILE="${LOG_DIR}/butterfly_unit.log"
...
iverilog -g2012 -Wall \
    ...
    "${ROOT_DIR}/rtl/ntt/butterfly_unit.v" \
    "${ROOT_DIR}/tb/unit/tb_butterfly_unit.v" 2>&1 | tee "${LOG_FILE}"

vvp "${SIM_OUT}" 2>&1 | tee -a "${LOG_FILE}"

grep -q "PASS tb_butterfly_unit" "${LOG_FILE}"
