...
SIM_OUT="${OUT_DIR}/tb_poly_basemul_montgomery.vvp"
LOG_FILE="${LOG_DIR}/poly_basemul_montgomery.log"
...
iverilog -g2012 -Wall \
    ...
    tb/unit/tb_poly_basemul_montgomery.v 2>&1 | tee "${LOG_FILE}"

vvp "${SIM_OUT}" 2>&1 | tee -a "${LOG_FILE}"

grep -q "PASS tb_poly_basemul_montgomery" "${LOG_FILE}"
