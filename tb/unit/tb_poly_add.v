...
module tb_poly_add;
    ...
    initial begin
        $dumpfile("sim/waves/poly_add.vcd");
        $dumpvars(0, tb_poly_add);
    end
    ...
    task record_fail;
        ...
        begin
            ...
            if (!first_fail_seen) begin
                first_fail_seen = 1;
                $display("FIRST_FAIL tb_poly_add: %0s", message);
            end
            $display("ERROR tb_poly_add: %0s", message);
        end
    endtask
        ...
        begin
            for (j = 0; j < `KYBER_N; j = j + 1) begin
                case (pattern_id)
                    ...
                    default: begin
                        a_poly[j] = ((j * 17) + 5) % `KYBER_Q;
                        b_poly[j] = ((j * 29) + 123) % `KYBER_Q;
                    end
                endcase

                expected[j] = expected_mod_add(a_poly[j], b_poly[j]);
            end
        end
    endtask
    ...
    initial begin
        ...
        for (pattern = 0; pattern < NUM_PATTERNS; pattern = pattern + 1) begin
            $display("INFO tb_poly_add: running pattern=%0d", pattern);
        ...
        $display("INFO tb_poly_add: pass_count=%0d fail_count=%0d",
                 pass_count, fail_count);

        if (fail_count == 0) begin
            $display("PASS tb_poly_add");
        end else begin
            $display("FAIL tb_poly_add");
        end
