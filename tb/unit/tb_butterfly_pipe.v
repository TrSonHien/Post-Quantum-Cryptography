...
module tb_butterfly_pipe;
    ...
    // t_expected    = (zeta_mont * v * 169) % 3329
    // out0_expected = (u + t_expected) % 3329
    // out1_expected = (u + 3329 - t_expected) % 3329
    function [11:0] get_expected_t;
        input [11:0] z;
        input [11:0] val_v;
        reg [63:0] temp;
        begin
            temp = z * val_v;
            temp = temp * 169;
            get_expected_t = temp % 3329;
        end
    endfunction

    function [11:0] get_expected_out0;
        input [11:0] val_u;
        input [11:0] val_t;
        reg [12:0] sum;
        begin
            sum = val_u + val_t;
            get_expected_out0 = sum % 3329;
        end
    endfunction

    function [11:0] get_expected_out1;
        input [11:0] val_u;
        input [11:0] val_t;
        reg [12:0] diff;
        begin
            if (val_u >= val_t) begin
                get_expected_out1 = val_u - val_t;
            end else begin
                diff = val_u + 13'd3329 - val_t;
                get_expected_out1 = diff % 3329;
            end
        end
    endfunction
    ...
    always @(negedge clk) begin
        if (rst_n) begin
            ...
            if (out_valid !== val_pipe[4]) begin
                ...
                t_expected    = get_expected_t(zeta_pipe[4], v_pipe[4]);
                out0_expected = get_expected_out0(u_pipe[4], t_expected);
                out1_expected = get_expected_out1(u_pipe[4], t_expected);

                if (out0 !== out0_expected) begin
                    fail_count = fail_count + 1;
                    $display("ERROR calculation out0: u=%d v=%d zeta=%d got=%d expected=%d",
                             u_pipe[4], v_pipe[4], zeta_pipe[4], out0, out0_expected);
    ...
    initial begin
        ...
        $display("INFO tb_butterfly_pipe: starting directed tests");

        // 1. Directed inputs (using actual zetas from rom: 2285, 2571, 2970)
        in_valid = 1;
        u = 0; v = 0; zeta_mont = 2285; @(posedge clk); #1;
        u = 1; v = 1; zeta_mont = 2285; @(posedge clk); #1;
        u = 3328; v = 3328; zeta_mont = 2571; @(posedge clk); #1;
        u = 1664; v = 1665; zeta_mont = 2970; @(posedge clk); #1;
        in_valid = 0; @(posedge clk); #1;

        repeat(7) @(posedge clk); #1;

        // 2. Bubble tests
        $display("INFO tb_butterfly_pipe: starting bubble tests");
        in_valid = 1; u = 100; v = 200; zeta_mont = 2285; @(posedge clk); #1;
        in_valid = 0; @(posedge clk); #1;
        in_valid = 1; u = 300; v = 400; zeta_mont = 2571; @(posedge clk); #1;
        in_valid = 0; @(posedge clk); #1;

        repeat(7) @(posedge clk); #1;

        // 3. Reset while transactions are pending
        $display("INFO tb_butterfly_pipe: starting reset test");
        in_valid = 1;
        u = 10; v = 20; zeta_mont = 2285; @(posedge clk); #1;
        u = 30; v = 40; zeta_mont = 2571; @(posedge clk); #1;
        u = 50; v = 60; zeta_mont = 2970; @(posedge clk); #1;
        ...
        $display("INFO tb_butterfly_pipe: running 2000 random inputs");
        in_valid = 1;
        ...
        #50;
        $display("INFO tb_butterfly_pipe: pass_count=%0d fail_count=%0d", pass_count, fail_count);

        if (fail_count == 0 && pass_count > 0) begin
            $display("PASS tb_butterfly_pipe");
            $finish;
        end else begin
            $display("FAIL tb_butterfly_pipe");
            $fatal(1, "Mismatches detected in Forward NTT butterfly pipeline");
        end
    end
