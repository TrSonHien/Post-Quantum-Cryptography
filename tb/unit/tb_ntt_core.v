...
module tb_ntt_core;
    ...
    task record_fail;
        ...
        begin
            ...
            if (!first_fail_seen) begin
                first_fail_seen = 1;
                $display("FIRST_FAIL tb_ntt_core: %0s", message);
            end
            $display("ERROR tb_ntt_core: %0s", message);
        end
    endtask
            ...
            for (len = 128; len >= 2; len = len >> 1) begin
                ...
                while (start_idx < 256) begin
                    ...
                    for (j = start_idx; j < start_idx + len; j = j + 1) begin
                        old_a = ref_poly[j];
                        t = fqmul_ref(zeta, ref_poly[j + len]);
                        ref_poly[j] = mod_q(old_a + t);
                        ref_poly[j + len] = mod_q(old_a - t);
                    end
                    start_idx = j + len;
                end
            end
    ...
    task wait_for_done;
        ...
        begin
            cycles = 0;
            done_pulses = 0;
            while (cycles < TOTAL_BUTTERFLIES + 20) begin
                @(negedge clk);
                #1;
                if (done) begin
        ...
        begin
            for (i = 0; i < N; i = i + 1) begin
                ...
                if (actual !== expected) begin
                    ...
                    if (!first_fail_seen) begin
                        first_fail_seen = 1;
                        $display("FIRST_FAIL tb_ntt_core: pattern=%0d index=%0d expected=%0d actual=%0d",
                                 pattern_id, i, expected, actual);
                    end
                    $display("ERROR tb_ntt_core: pattern=%0d index=%0d expected=%0d actual=%0d",
                             pattern_id, i, expected, actual);
                end else begin
    ...
    task run_pattern;
        input integer pattern_id;
        begin
            $display("INFO tb_ntt_core: starting pattern %0d", pattern_id);
            apply_reset();
            init_input_pattern(pattern_id);
            load_input_poly();
            compute_reference_ntt();
            start_core();
            wait_for_done();
            check_output_poly(pattern_id);
            $display("INFO tb_ntt_core: completed pattern %0d", pattern_id);
        end
    endtask

    initial begin
        $dumpfile("sim/waves/ntt_core.vcd");
        ...
        $display("INFO tb_ntt_core: pass_count=%0d fail_count=%0d", pass_count, fail_count);
        if (fail_count == 0) begin
            $display("PASS tb_ntt_core");
        end else begin
            $display("FAIL tb_ntt_core");
        end
        $finish;
    end
