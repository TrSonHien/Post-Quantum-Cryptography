...
|---|---|---|---|---|---|---|---|
| M7-KG | `*.kpke_keygen` (1 instance) | d/rho/sigma 8x32 each; ek 8x1184; dk 8x1152; six 12x768 vectors; dot 12x256; control = 78,036 bits | secret+ephemeral+designated; K-PKE KeyGen | final output accepted | LI / 1,184-step local scrub, child reset is not erase | controller; local scan plus explicit child requests/acks | max(1186, children)+2 / K-PKE differential+zeroize TB / local complete, hierarchy missing |
| M7-EN | `*.kpke_encrypt` (2 instances) | per instance ek 8x1184; m/r/rho 8x32; c 8x1088; six 12x768 and six 12x256 arrays; aggregate 185,708 bits | secret+ephemeral+designated c; K-PKE Encrypt/re-encrypt | final c accepted | LI / 1,184-step local scrub only | controller; scan plus all child requests/acks | max(1186, children)+2 / Encrypt differential+zeroize TB / local complete, hierarchy missing |
| M7-DE | `*.kpke_decrypt` (1 instance) | dk 8x1152; c 8x1088; msg 8x32; three 12x768 and four 12x256 arrays; 58,237 bits | secret+ephemeral+designated m'; K-PKE Decrypt | message accepted by Decaps | LI / 1,152-step local scrub only | controller; scan plus all child requests/acks | max(1154, children)+2 / Decrypt differential+zeroize TB / local complete, hierarchy missing |
| M7-MAT | `*.kpke_matrix_row_sampler` (3) | rho_q 256, row/transpose/element and control; 807 bits | public; matrix A-hat | row captured | reset currently directly clears scalar payload / no explicit zeroize | sampler; metadata direct clear, optional child public scrub | 6+XOF / matrix/transpose regression / public-matrix exception |
| M7-NOISE | `*.kpke_noise_vector_sampler` (3) | seed_q 256, nonce/eta/element/control; 852 bits | secret+ephemeral; KeyGen/Encrypt noise | sampled vector captured | reset currently clears scalars / no child ack | sampler; direct clear+noise child zeroize | child+4 / nonce regression+hierarchy readback / missing |
...
| P-XFORM | `*.poly_transform_pipe` | 7; transfer/index/result metadata, 224 bits plus child storage | secret/ephemeral; NTT/INTT adapters | LI / none | transform owner; child workspace/core requests and ack join | max(children)+4 / ordinary transform+abort / missing |
| PV-ELEM | `*.polyvec_elementwise_pipe` | 4; state/index/result metadata, 140 bits plus child storage | secret/ephemeral; polyvec NTT and arithmetic | LI / none | vector owner; child/vector-workspace requests and ack join | max(children)+4 / ordinary polyvec tests / missing |
| P-BIN | `*.poly_binary_pipe` | 4; state/pair/result metadata, 112 bits plus three workspaces/pipelines | secret/ephemeral; poly add/sub | LI / none | binary owner; workspace scans plus two lane flushes | 134 / add/sub oracle and hierarchy readback / missing |
| PV-BM | `*.polyvec_basemul_acc_pipe` | 4; state/index/result metadata, 172 bits plus workspaces/child | secret/ephemeral; dot products | LI / none | accumulator owner; all workspace/child requests and ack join | 140 / BaseCaseMultiply differential / missing |
| P-BM | `*.poly_basemul_pipe` | 4; pair/index/result metadata, 132 bits plus 3 workspaces and lanes | secret/ephemeral; MultiplyNTTs | LI / none | basemul owner; workspace scans plus pipeline flush | 136 / ordinary basemul+readback / missing |
...
| MOD-NMUL | `mod_mul_normal_pipe` | 20; 500 bits | secret/ephemeral pipeline | same | zero flush | 8 / normal multiply TB / missing |
...
| BAR | `barrett_reduce_pipe` | 20; 3,620 bits | secret/ephemeral pipeline | same | zero flush | 5 / reduction TB / missing |
...
Bottom-up owner closure has begun without changing mathematical outputs. M6
codec, PRF/XOF, CBD/noise, and SampleNTT/parser owners now implement explicit
physical clear and hierarchical child acknowledgement. The nonrecursive M6
ordinary regression passes 15/15 with 664,856 checks, and the dedicated M6
zeroize tier passes 3/3 with reset interruption and clean restart. This is not
hierarchical M8 completion: transform adapters, K-PKE controllers, M8
controllers, public wrappers, and the unified boot/abort scrub FSM remain open.
