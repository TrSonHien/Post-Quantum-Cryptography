 ...
 * Latency / completion: fixed 3 pipeline stages.
...
module barrett_reduce_pipe
    ...
    always @(posedge clk) begin
        if ((zeroize_req === 1'b1) || zeroize_busy)
            r <= 0;
    ...
    always @(posedge clk) begin
        if (val_d2 && !zeroize_busy && zeroize_req !== 1'b1) begin
            if (prod2 > a_d2) begin
                $display("ASSERTION FAILED in barrett_reduce_pipe: quotient*q (%0d) > a (%0d)",
                         prod2, a_d2);
                $fatal(1);
            end
            if (remainder0 >= 13'd6658) begin
                $display(
                    "ASSERTION FAILED in barrett_reduce_pipe: remainder0 (%0d) >= 2*q (6658)",
                    remainder0);
                $fatal(1);
            end
            if (r_next >= 12'd3329) begin
                $display("ASSERTION FAILED in barrett_reduce_pipe: final r (%0d) >= q (3329)",
                         r_next);
                $fatal(1);
            end
        end
