...
module keccak_sponge_ctx
    ...
    function automatic[1599 : 0] xor_byte;
        ...
        begin
            t = s;
            t[8 * offset +: 8] = t[8 * offset +: 8] ^ value;
            xor_byte = t;
        end
    endfunction
    ...
    keccak_f1600_core u_perm(
                              ...
                              .zeroize_done(perm_zeroize_done));
        ...
        if (!rst_n) begin
            ...
            if (zeroize_req === 1'b1) begin
                ...
                if (init_valid && !init_ready)
                    error <= 1;
                if (absorb_valid && (!context_valid || !absorb_phase))
                    error <= 1;
                if (finalize_valid && (!context_valid || !absorb_phase))
                    error <= 1;
                if (squeeze_req_valid && (!context_valid || !squeeze_phase))
                    error <= 1;
                ...
                if (perm_active && perm_done) begin
                    ...
                    if (squeeze_offset >= rate_bytes) begin
                        ...
                        if (squeeze_offset == rate_bytes - 1'b1) begin
                            ...
                            squeeze_offset <= squeeze_offset + 1'b1;
