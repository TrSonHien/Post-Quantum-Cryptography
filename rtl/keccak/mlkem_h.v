...
module mlkem_h
    ...
    wire [3 : 0] core_out_keep;
    wire core_out_last;
    wire accept_zeroize = (zeroize_req === 1'b1);
    assign busy = core_busy | zeroize_busy;
    assign done = core_done && !zeroize_busy;
    ...
    assign out_keep = core_out_keep;
    assign out_last = core_out_last;
    assign error = !zeroize_busy && core_error;
    sha3_256_stream u(.clk(clk),
                      .rst_n(rst_n),
                      .cmd_valid(cmd_valid && !zeroize_busy),
                      ...
                      .out_keep(core_out_keep),
                      .out_last(core_out_last),
                      .busy(core_busy),
                      .done(core_done),
                      .error(core_error),
                      .zeroize_req(zeroize_busy),
                      .zeroize_done(core_zeroize_done));
    always @(posedge clk) begin
        zeroize_done <= 0;
        if (!rst_n)
            zeroize_busy <= 0;
        else if (accept_zeroize && !zeroize_busy)
            zeroize_busy <= 1;
        else if (zeroize_busy && core_zeroize_done) begin
            zeroize_busy <= 0;
            zeroize_done <= 1;
        end
    end
