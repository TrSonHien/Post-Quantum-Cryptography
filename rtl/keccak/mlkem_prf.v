...
module mlkem_prf
    ...
    reg [1 : 0] eta_reg;
    reg [3 : 0] word_index;
    wire hc_ready, hi_ready, hov, hbusy, hdone, herror, hash_zeroize_done;
    wire [31 : 0] hod;
    wire [3 : 0] hok;
    wire hol;
    reg child_zeroize_req;
    wire [31 : 0] feed_data = (word_index < 8) ? seed_reg[32 * word_index +: 32] : {24'h0, nonce_reg};
    wire [3 : 0] feed_keep = (word_index < 8) ? 4'b1111 : 4'b0001;
    wire feed_last = (word_index == 8);
    assign ready = (state == IDLE) && !zeroize_busy;
    assign out_valid = (state == OUTPUT) && hov && !zeroize_busy;
    assign out_data = hod;
    assign out_keep = hok;
    assign out_last = hol;
    keccak_hash_stream u_hash(.clk(clk),
                              .rst_n(rst_n),
                              .cmd_valid(state == CMD && !zeroize_busy),
                              .cmd_ready(hc_ready),
                              .mode(2'd3),
                              .msg_len_bytes(33),
                              .out_len_bytes((eta_reg == 2) ? 128 : 192),
                              .in_valid(state == FEED && !zeroize_busy),
                              .in_ready(hi_ready),
                              .in_data(feed_data),
                              .in_keep(feed_keep),
                              .in_last(feed_last),
                              .out_valid(hov),
                              .out_ready((state == OUTPUT) && out_ready && !zeroize_busy),
                              .out_data(hod),
                              .out_keep(hok),
                              .out_last(hol),
                              .busy(hbusy),
