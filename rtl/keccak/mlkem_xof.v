...
module mlkem_xof
    (
        ...
        input wire [7 : 0] index0,
        input wire [7 : 0] index1,
        input wire use_generic_input,
        input wire [271 : 0] generic_input,
    ...
    reg [271 : 0] input_reg;
    reg [3 : 0] word_index;
    wire ci_ready, cf_ready, cv, cap, csp, cb, cd, ce, ca_ready, cs_ready, cov, ctx_zeroize_done;
    wire [31 : 0] cod;
    wire [3 : 0] cok;
    wire col;
    ...
    assign squeeze_req_ready = !zeroize_busy && (state == ACTIVE) && cs_ready;
    assign out_valid = !zeroize_busy && (state == ACTIVE) && cov;
    assign out_data = cod;
    assign out_keep = cok;
    assign out_last = col;
    assign context_valid = !zeroize_busy && (state == ACTIVE) && cv;
    assign busy = zeroize_busy || !(state == IDLE || state == ACTIVE) || cb;
    keccak_sponge_ctx u_ctx(.clk(clk),
                            .rst_n(rst_n),
                            .init_valid(state == CTX_INIT && !zeroize_busy),
                            .init_ready(ci_ready),
                            .mode(2'd2),
                            .finalize_valid(state == FINAL && !zeroize_busy),
                            ...
                            .done(cd),
                            .error(ce),
                            .absorb_valid(state == FEED && !zeroize_busy),
                            .absorb_ready(ca_ready),
                            .absorb_data(feed_data),
                            .absorb_keep(feed_keep),
                            .squeeze_req_valid((state == ACTIVE) && squeeze_req_valid && !zeroize_busy),
                            .squeeze_req_ready(cs_ready),
                            .squeeze_len_bytes(squeeze_len_bytes),
                            .out_valid(cov),
                            .out_ready((state == ACTIVE) && out_ready && !zeroize_busy),
                            .out_data(cod),
                            .out_keep(cok),
                            .out_last(col),
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (ce)
                error <= 1;
            if (init_valid && !init_ready)
                error <= 1;
            if (state == ACTIVE && cov && out_ready && col)
                done <= 1;
            case (state)
                IDLE, ACTIVE:
                    if (init_valid && init_ready) begin
                        input_reg <= use_generic_input ? generic_input : {index1, index0, seed};
                        word_index <= 0;
                        error <= 0;
                        state <= CTX_INIT;
                    end
