...
module keccak_hash_stream
    ...
    reg input_grace;
    reg [1 : 0] mode_reg;
    reg [31 : 0] msg_remaining, out_len_reg;
    wire ci_ready, cf_ready, ca_ready, cs_ready, cv, cap, csp, cbusy, cdone, cerror;
    wire cov;
    wire [31 : 0] cod;
    wire [3 : 0] cok;
    wire col;
    wire ci_valid = (state == INIT);
    wire cf_valid = (state == FINAL);
    wire cs_valid = (state == REQ_SQ);
    wire [2 : 0] expected_count = (msg_remaining >= 4) ? 3'd4 : msg_remaining[2 : 0];
    wire [3 : 0] expected_keep = (expected_count == 1) ? 4'b0001 : (expected_count == 2) ? 4'b0011
                                                               : (expected_count == 3)   ? 4'b0111
                                                                                         : 4'b1111;
    ...
    assign out_valid = (state == OUTPUT) && cov;
    assign out_data = cod;
    assign out_keep = cok;
    assign out_last = col;
    ...
    keccak_sponge_ctx u_ctx(
                              ...
                              .init_ready(ci_ready),
                              .mode(mode_reg),
                              .finalize_valid(cf_valid),
                              .finalize_ready(cf_ready),
                              .context_valid(cv),
                              .absorb_phase(cap),
                              .squeeze_phase(csp),
                              .busy(cbusy),
                              .done(cdone),
                              .error(cerror),
                              .absorb_valid(ca_valid),
                              .absorb_ready(ca_ready),
                              .absorb_data(in_data),
                              .absorb_keep(in_keep),
                              .squeeze_req_valid(cs_valid),
                              .squeeze_req_ready(cs_ready),
                              .squeeze_len_bytes(out_len_reg),
                              .out_valid(cov),
                              .out_ready((state == OUTPUT) && out_ready),
                              .out_data(cod),
                              .out_keep(cok),
                              .out_last(col),
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            out_len_reg <= 0;
            input_grace <= 0;
        end else if (zeroize_req === 1'b1) begin
            state <= IDLE;
            busy <= 0;
            done <= 0;
            error <= 0;
            ...
            if (cerror) begin
                error <= 1;
`ifdef M5_DEBUG $display("HASH_ERR ctx");
`endif
            end
            if (cmd_valid && busy) begin
                error <= 1;
`ifdef M5_DEBUG $display("HASH_ERR cmd_busy");
`endif
            end
            if (in_valid && busy && state >= FINAL && !input_grace) begin
                error <= 1;
`ifdef M5_DEBUG $display("HASH_ERR extra state=%0d", state);
`endif
            end
            case (state)
                IDLE:
                    if (cmd_valid) begin
                        if ((mode == 0 && out_len_bytes != 32) || (mode == 1 && out_len_bytes != 64) ||
                            (mode >= 2 && out_len_bytes == 0)) begin
                            error <= 1;
                            busy <= 0;
                        end else begin
                            mode_reg <= mode;
                            msg_remaining <= msg_len_bytes;
                            out_len_reg <= out_len_bytes;
                            busy <= 1;
                            error <= 0;
                            input_grace <= 0;
                            state <= INIT;
                ...
                INIT:
                    if (ci_ready)
                        state <= (msg_remaining == 0) ? FINAL : INPUT;
                INPUT:
                    if (in_valid && in_ready) begin
                        if (!input_ok) begin
                            error <= 1;
`ifdef M5_DEBUG $display("HASH_ERR input expected=%b/%b got=%b/%b rem=%0d", expected_keep, expected_last, in_keep, in_last, msg_remaining);
`endif busy <= 0;
                            state <= IDLE;
                        end else begin
                            msg_remaining <= msg_remaining - expected_count;
