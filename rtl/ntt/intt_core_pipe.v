 ...
 * Submodules: fixed_latency_delay, intt_butterfly_pipe, intt_scaler_pipe, intt_scheduler_pipe, ntt_pingpong_banks, zetas_rom.
...
module intt_core_pipe
    ...
    reg [5 : 0] child_done_seen;
    reg [6 : 0] scrub_addr;
    reg local_scrub_done;
    ...
    intt_scheduler_pipe u_scheduler(
                                ...
                                .src_u_idx(sched_u_idx),
                                .src_v_idx(sched_v_idx),
                                ...
                                .zeroize_busy(sched_zeroize_busy),
                                .zeroize_done(sched_zeroize_done));
    ...
    wire [7 : 0] mem_src_index0 = intt_read_req ? sched_u_idx : scale_read_req ? {1'b0, scale_pair_count[6 : 0]}
                                                                               : result_rd_idx;
    wire [7 : 0] mem_src_index1 = intt_read_req ? sched_v_idx : scale_read_req ? {1'b1, scale_pair_count[6 : 0]}
                                                                               : (result_rd_idx ^ 8'h80);
    ...
    wire [11 : 0] zeta_now;
    zetas_rom u_zetas_rom(.inverse(1'b0),
    ...
    fixed_latency_delay #(.PAYLOAD_WIDTH(12),
        ...
        u_zeta_delay(
                ...
                .zeroize_req(child_zeroize_req),
                .zeroize_busy(zeta_zeroize_busy),
                .zeroize_done(zeta_zeroize_done));
    ...
    fixed_latency_delay #(
        ...
        u_write_meta_delay(
                ...
                .rst_n(rst_n),
                .in_valid(intt_read_req),
                .in_payload(issue_meta),
                .in_metadata(1'b0),
                .out_valid(write_meta_valid),
                .out_payload(write_meta),
                .out_metadata(write_meta_unused),
                .zeroize_req(child_zeroize_req),
                .zeroize_busy(meta_zeroize_busy),
                .zeroize_done(meta_zeroize_done));
    ...
    intt_butterfly_pipe u_butterfly(
                                .clk(clk),
                                .rst_n(rst_n),
                                .in_valid(intt_bfly_in_valid),
                                .u(mem_src_data0),
                                .v(mem_src_data1),
                                .zeta_mont(zeta_for_bfly),
                                .out_valid(intt_bfly_out_valid),
                                .out0(intt_bfly_out0),
                                .out1(intt_bfly_out1),
                                .zeroize_req(child_zeroize_req),
                                .zeroize_busy(bfly_zeroize_busy),
                                .zeroize_done(bfly_zeroize_done));
    ...
    intt_scaler_pipe u_scaler(
                             .clk(clk),
                             .rst_n(rst_n),
                             .in_valid(scale_input_valid),
                             .in0(mem_src_data0),
                             .in1(mem_src_data1),
                             .pair_idx(scale_response_pair_d[6 : 0]),
                             .last_in(scale_response_pair_d == 8'd127),
                             .out_valid(scale_out_valid),
                             .out0(scale_out0),
                             .out1(scale_out1),
                             .pair_idx_out(scale_out_pair),
                             .last_out(scale_out_last),
                             .zeroize_req(child_zeroize_req),
                             .zeroize_busy(scale_zeroize_busy),
                             .zeroize_done(scale_zeroize_done));
    ...
    wire [7 : 0] mem_dst_index0 = intt_write_setup_valid ? write_u_idx : scale_out_valid ? {1'b0, scale_out_pair}
                                                                                         : preload_u_idx;
    wire [7 : 0] mem_dst_index1 = intt_write_setup_valid ? write_v_idx : scale_out_valid ? {1'b1, scale_out_pair}
                                                                                         : preload_v_idx;
    wire [2 : 0] mem_dst_pair_bit = intt_write_setup_valid ? write_pair_bit : scale_out_valid ? 3'd7
                                                                                              : 3'd1;
    wire [2 : 0] mem_dst_addr_bit = intt_write_setup_valid ? write_addr_bit : scale_out_valid ? 3'd7
                                                                                              : 3'd1;
    wire mem_dst_xor_layout = intt_write_setup_valid ? write_xor : scale_out_valid ? 1'b0
                                                                                   : 1'b0;
    wire [DATA_WIDTH - 1 : 0] mem_dst_data0 = intt_write_setup_valid ? intt_bfly_out0 : scale_out_valid ? scale_out0
                                                                                                        : preload_first[preload_slot];
    wire [DATA_WIDTH - 1 : 0] mem_dst_data1 = intt_write_setup_valid ? intt_bfly_out1 : scale_out_valid ? scale_out1
                                                                                                        : preload_coeff;

    wire role_select;
    ntt_pingpong_banks #(.DATA_WIDTH(DATA_WIDTH))
        u_banks(
                .clk(clk),
                .rst_n(rst_n),
                ...
                .zeroize_req(child_zeroize_req),
                .zeroize_busy(banks_zeroize_busy),
                .zeroize_done(banks_zeroize_done));
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            error <= 0;
            final_stage_latched <= 0;
            results_valid <= 0;
            preload_pair_count <= 0;
            scale_pair_count <= 0;
            intt_write_setup_valid_d <= 0;
            intt_write_last_stage_d <= 0;
            intt_write_last_transform_d <= 0;
            scale_response_pair_d <= 0;
            scale_response_valid_d <= 0;
            scale_write_setup_valid_d <= 0;
            scale_write_last_d <= 0;
            accepted_read_count <= 0;
            ram_response_count <= 0;
            butterfly_input_count <= 0;
            butterfly_output_count <= 0;
            committed_write_count <= 0;
            stage_drain_count <= 0;
            stage_advance_count <= 0;
            stage_role_swap_count <= 0;
            total_role_swap_count <= 0;
            pending_reads <= 0;
            pending_butterflies <= 0;
            pending_writes <= 0;
            inverse_cycle_count <= 0;
            transform_cycle_count <= 0;
            scale_first_issue_cycle <= 0;
            scale_final_issue_cycle <= 0;
            scale_final_commit_cycle <= 0;
            scale_swap_cycle <= 0;
            done_cycle <= 0;
            scale_read_count <= 0;
            scale_response_count <= 0;
            scale_output_pair_count <= 0;
            scale_write_count <= 0;
            scale_coefficient_input_count <= 0;
            scale_coefficient_output_count <= 0;
            scale_pending_reads <= 0;
            scale_pending_mults <= 0;
            scale_pending_writes <= 0;
            zeroize_busy <= 0;
            zeroize_done <= 0;
            child_zeroize_req <= 0;
            child_done_seen <= 0;
            ...
            if (zeroize_req === 1'b1 && !zeroize_busy) begin
                ...
                if (!local_scrub_done) begin
                    preload_first[scrub_addr] <= 0;
                    preload_first_seen[scrub_addr] <= 0;
                    preload_pair_written[scrub_addr] <= 0;
                ...
                if (busy)
                    transform_cycle_count <= transform_cycle_count + 1;
                if (busy && state <= ST_STAGE_ADVANCE)
                    inverse_cycle_count <= inverse_cycle_count + 1;

                if (intt_read_req) begin
                ...
                if (intt_write_setup_valid)
                    pending_writes <= pending_writes + 1;
                if (intt_write_commit_valid) begin
                    committed_write_count <= committed_write_count + 1;
                    pending_writes <= pending_writes - 1;
                end
                if (intt_read_req && intt_bfly_in_valid)
                    pending_reads <= pending_reads;
                if (intt_bfly_in_valid && intt_bfly_out_valid)
                    pending_butterflies <= pending_butterflies;
                if (intt_write_setup_valid && intt_write_commit_valid)
                    pending_writes <= pending_writes;

                if (scale_read_req) begin
                    scale_read_count <= scale_read_count + 1;
                    scale_pending_reads <= scale_pending_reads + 1;
                    if (scale_pair_count == 0)
                        scale_first_issue_cycle <= transform_cycle_count + 1;
                    if (scale_pair_count == 127)
                        scale_final_issue_cycle <= transform_cycle_count + 1;
                end
                if (scale_input_valid) begin
                    ...
                    scale_pending_reads <= scale_pending_reads - 1;
                    scale_pending_mults <= scale_pending_mults + 1;
                end
                if (scale_out_valid) begin
                    ...
                    scale_pending_mults <= scale_pending_mults - 1;
                    scale_pending_writes <= scale_pending_writes + 1;
                end
                if (scale_write_commit_valid) begin
                    scale_write_count <= scale_write_count + 1;
                    scale_pending_writes <= scale_pending_writes - 1;
                    if (scale_write_last_d)
                        scale_final_commit_cycle <= transform_cycle_count + 1;
                end
                if (scale_read_req && scale_input_valid)
                    scale_pending_reads <= scale_pending_reads;
                if (scale_input_valid && scale_out_valid)
                    scale_pending_mults <= scale_pending_mults;
                if (scale_out_valid && scale_write_commit_valid)
                    scale_pending_writes <= scale_pending_writes;
                ...
                if (preload_pair_wr_en) begin
                    results_valid <= 0;
                    if (!preload_first_seen[preload_slot])
                        error <= 1;
                ...
                case (state)
                    ...
                    ST_STAGE_ADVANCE: begin
                        ...
                        if (final_stage_latched) begin
                            scale_pair_count <= 0;
                            state <= ST_SCALE_ISSUE;
                        end else
                            state <= ST_RUN;
                    end
                    ST_SCALE_ISSUE: begin
                        if (scale_pair_count == 8'd127)
                            state <= ST_SCALE_DRAIN;
                        else
                            scale_pair_count <= scale_pair_count + 1;
                    end
                    ST_SCALE_DRAIN:
                        if (scale_drain_ready)
                            state <= ST_SCALE_SWAP;
                    ST_SCALE_SWAP: begin
                        total_role_swap_count <= total_role_swap_count + 1;
