...
module intt_core #(
    ...
    assign buf_rd_addr_a = (state == ST_RUN)   ? gen_addr_a :
                           (state == ST_SCALE) ? scale_idx  : read_addr;
    ...
    assign buf_wr_addr_a = (state == ST_RUN)   ? gen_addr_a :
                           (state == ST_SCALE) ? scale_idx  : load_addr;

    assign buf_wr_data_a = (state == ST_RUN)   ? sum     :
                           (state == ST_SCALE) ? mul_out : load_data;
