 ...
 * Submodules: fixed_latency_delay, mod_mul_pipe.
...
module intt_scaler_pipe
    ...
    mod_mul_pipe u_lane0(
                         ...
                         .zeroize_busy(lane0_zeroize_busy),
                         .zeroize_done(lane0_zeroize_done));
    mod_mul_pipe u_lane1(
                         ...
                         .r(lane1_out),
                         .zeroize_req(child_zeroize_req),
                         .zeroize_busy(lane1_zeroize_busy),
                         .zeroize_done(lane1_zeroize_done));
    fixed_latency_delay #(
            .PAYLOAD_WIDTH(8),
            .METADATA_WIDTH(1),
            .LATENCY(MOD_MUL_LATENCY))
        u_metadata_delay(
                .clk(clk),
                .rst_n(rst_n),
                .in_valid(in_valid && !zeroize_busy),
                .in_payload({pair_idx, last_in}),
                .in_metadata(1'b0),
                .out_valid(metadata_valid),
                .out_payload(metadata_out),
    ...
    assign out0 = lane0_out;
    assign out1 = lane1_out;
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (&(child_done_seen | {meta_zeroize_done, lane1_zeroize_done, lane0_zeroize_done})) begin
                zeroize_busy <= 0;
                zeroize_done <= 1;
            end
        end
