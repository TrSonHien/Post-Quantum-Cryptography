...
module kpke_encrypt(
 ...
 MSG_START=41,MSG_FEED=42,MSG_WAIT=43,PACKU_START=44,PACKU_FEED=45,PACKU_WAIT=46,PACKV_START=47,PACKV_FEED=48,PACKV_WAIT=49,OUTPUT=50;
 ...
 polyvec_decode12_pipe dec(clk,rst_n,dec_start,NTT,dec_busy,dec_done,dec_error,state==DEC_FEED,dec_in_ready,{ek[word_count*4+3],ek[word_count*4+2],ek[word_count*4+1],ek[word_count*4]},4'hf,word_count==287,dec_valid,1'b1,dec_coeff,dec_idx,dec_domain,dec_poly,dec_nc);
 assign nv_start=state==NY_START||state==NE_START;
 kpke_noise_vector_sampler nv(clk,rst_n,nv_start,r_packed,(state==NE_START||state==NE_WAIT)?8'd3:8'd0,2'd2,nv_busy,nv_done,nv_error,nv_valid,1'b1,nv_coeff,nv_idx,nv_poly,nv_domain,nv_nonce,nv_next,nv_samples);
 assign e2_start=state==E2_START;
 mlkem_noise_sampler e2s(clk,rst_n,e2_start,r_packed,8'd6,2'd2,e2_busy,e2_done,e2_error,e2_valid,1'b1,e2_coeff,e2_idx,e2_domain);
 assign n_begin=state==N_BEGIN;assign n_we=state==N_LOAD;assign n_start=state==N_START;assign n_rr=state==N_READ;
 polyvec_ntt_pipe pn(clk,rst_n,n_begin,1'b0,elem,NORMAL,n_we,idx,y[{elem,idx}],n_lr,n_start,n_busy,n_done,n_error,n_rr,elem,idx,n_rv,n_rc,n_rd,n_complete,state==N_RELEASE);
 assign mat_start=state==M_START;
 kpke_matrix_row_sampler mr(clk,rst_n,mat_start,rho_packed,row,1'b1,mat_busy,mat_done,mat_error,mat_valid,1'b1,mat_coeff,mat_idx,mat_poly,mat_domain,mi0,mi1,mat_samples);
 ...
 poly_intt_pipe pi(clk,rst_n,i_begin,NTT,i_we,idx,dot[idx],i_lr,i_start,i_busy,i_done,i_error,state==I_READ,idx,i_rv,i_rc,i_rd,i_complete,state==I_RELEASE);
 ...
 poly_add_pipe pa(clk,rst_n,a_begin,operand,NORMAL,state==A_LOAD&&!operand,idx,a0,state==A_LOAD&&operand,idx,a1,a_lr,a_start,a_busy,a_done,a_error,state==A_READ,idx,a_rv,a_rc,a_rd,a_complete,state==A_RELEASE);
 assign msg_start=state==MSG_START;
 message_to_poly_pipe mp(clk,rst_n,msg_start,msg_busy,msg_done,msg_error,state==MSG_FEED,msg_in_ready,{m[word_count*4+3],m[word_count*4+2],m[word_count*4+1],m[word_count*4]},4'hf,word_count==7,msg_valid,1'b1,msg_coeff,msg_idx,msg_domain);
 assign pu_start=state==PACKU_START;
 polyvec_compress_encode10_pipe pcu(clk,rst_n,pu_start,NORMAL,pu_busy,pu_done,pu_error,state==PACKU_FEED,pu_in_ready,u[linear],pu_valid,1'b1,pu_data,pu_keep,pu_last,pu_poly);
 assign pv_start=state==PACKV_START;
 poly_compress_encode_pipe #(.D(4)) pcv(clk,rst_n,pv_start,NORMAL,pv_busy,pv_done,pv_error,state==PACKV_FEED,pv_in_ready,v[idx],pv_valid,1'b1,pv_data,pv_keep,pv_last);
 always@(posedge clk)begin
  if(!rst_n)begin state<=IDLE;busy<=0;done<=0;error<=0;noncanonical_seen<=0;cycle_count<=0;word_count<=0;linear<=0;linear_d<=0;idx<=0;elem<=0;row<=0;operand<=0;vbranch<=0;add_second<=0;out_word<=0;pack_word<=0;sample_ntt_operations<=0;noise_operations<=0;polyvec_ntt_operations<=0;dot_operations<=0;intt_operations<=0;poly_add_operations<=0;message_operations<=0;end else begin
   done<=0;if(busy)cycle_count<=cycle_count+1;if(cmd_valid&&!cmd_ready)error<=1;if(in_valid&&!in_ready)error<=1;if(dec_error||nv_error||e2_error||n_error||mat_error||b_error||i_error||a_error||msg_error||pu_error||pv_error)error<=1;if(dec_nc)noncanonical_seen<=1;
   ...
   case(state)
    ...
    OUTPUT:if(out_valid&&out_ready)begin if(out_word==271)begin busy<=0;done<=1;state<=IDLE;end else out_word<=out_word+1'b1;end default:state<=IDLE;
   endcase
  end
 end
endmodule
