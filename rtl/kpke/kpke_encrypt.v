 ...
 * Submodules: kpke_matrix_row_sampler, kpke_noise_vector_sampler, message_to_poly_pipe, mlkem_noise_sampler, poly_add_pipe, poly_compress_encode_pipe, poly_intt_pipe, polyvec_basemul_acc_pipe, polyvec_compress_encode10_pipe, polyvec_decode12_pipe, polyvec_ntt_pipe.
...
module kpke_encrypt
    ...
    reg [6 : 0] state;
    reg [7 : 0] ek[0 : 1183], m[0 : 31], r[0 : 31], rho[0 : 31], c[0 : 1087];
    ...
    reg [9 : 0] word_count, linear, linear_d, out_word, pack_word;
    reg [7 : 0] idx;
    reg [1 : 0] elem, row;
    reg operand, vbranch, add_second;
    integer j;
    reg [10 : 0] scrub_addr;
    wire zeroize_req = (zeroize === 1'b1);
    reg [10 : 0] child_zeroize_req, child_zeroized;
    wire [10 : 0] child_zeroize_busy, child_zeroize_done;
    wire dec_start, dec_busy, dec_done, dec_error, dec_in_ready, dec_valid, dec_nc;
    wire [11 : 0] dec_coeff;
    wire [7 : 0] dec_idx;
    wire [1 : 0] dec_domain, dec_poly;
    wire nv_start, nv_busy, nv_done, nv_error, nv_valid;
    wire [11 : 0] nv_coeff;
    wire [7 : 0] nv_idx, nv_nonce, nv_next;
    wire [1 : 0] nv_poly, nv_domain;
    wire [2 : 0] nv_samples;
    wire e2_start, e2_busy, e2_done, e2_error, e2_valid;
    wire [11 : 0] e2_coeff;
    wire [7 : 0] e2_idx;
    wire [1 : 0] e2_domain;
    wire n_begin, n_we, n_lr, n_start, n_busy, n_done, n_error, n_rv, n_rr, n_complete;
    wire [11 : 0] n_rc;
    wire [1 : 0] n_rd;
    ...
    wire [11 : 0] i_rc;
    wire [1 : 0] i_rd;
    wire a_begin, a_lr, a_start, a_busy, a_done, a_error, a_rv, a_complete;
    wire [11 : 0] a0, a1, a_rc;
    wire [1 : 0] a_rd;
    wire msg_start, msg_busy, msg_done, msg_error, msg_in_ready, msg_valid;
    wire [11 : 0] msg_coeff;
    wire [7 : 0] msg_idx;
    wire [1 : 0] msg_domain;
    wire pu_start, pu_busy, pu_done, pu_error, pu_in_ready, pu_valid;
    wire [31 : 0] pu_data;
    wire [3 : 0] pu_keep;
    wire pu_last;
    wire [1 : 0] pu_poly;
    wire pv_start, pv_busy, pv_done, pv_error, pv_in_ready, pv_valid;
    wire [31 : 0] pv_data;
    wire [3 : 0] pv_keep;
    wire pv_last;
    ...
    assign out_last = state == OUTPUT && out_word == 271;
    assign out_data = {c[out_word * 4 + 3], c[out_word * 4 + 2], c[out_word * 4 + 1], c[out_word * 4]};
    assign dec_start = state == DEC_START;
    polyvec_decode12_pipe dec(clk,
                              rst_n,
                              dec_start,
                              NTT,
                              dec_busy,
                              ...
                              state == DEC_FEED,
                              dec_in_ready,
                              {ek[word_count * 4 + 3], ek[word_count * 4 + 2], ek[word_count * 4 + 1], ek[word_count * 4]},
                              4'hf,
                              word_count == 287,
                              dec_valid,
                              1'b1,
    ...
    kpke_noise_vector_sampler nv(clk,
                                 rst_n,
                                 nv_start,
                                 r_packed,
                                 (state == NE_START || state == NE_WAIT) ? 8'd3 : 8'd0,
                                 2'd2,
                                 nv_busy,
                                 nv_done,
                                 nv_error,
    ...
    kpke_matrix_row_sampler mr(clk,
                               rst_n,
                               mat_start,
                               rho_packed,
                               row,
    ...
    assign a_begin = state == A_BEGIN;
    assign a_start = state == A_START;
    assign a0 = add_second ? temp[idx] : product[idx];
    assign a1 = vbranch ? (add_second ? mu[idx] : e2[idx]) : e1[{row, idx}];
    poly_add_pipe pa(clk,
                     rst_n,
                     a_begin,
                     operand,
                     NORMAL,
                     state == A_LOAD && !operand,
                     idx,
                     a0,
                     state == A_LOAD && operand,
                     idx,
    ...
    message_to_poly_pipe mp(clk,
                            ...
                            state == MSG_FEED,
                            msg_in_ready,
                            {m[word_count * 4 + 3], m[word_count * 4 + 2], m[word_count * 4 + 1], m[word_count * 4]},
                            4'hf,
                            word_count == 7,
                            msg_valid,
                            1'b1,
                            ...
                            msg_domain,
                            child_zeroize_req[8],
                            child_zeroize_busy[8],
                            child_zeroize_done[8]);
    assign pu_start = state == PACKU_START;
    polyvec_compress_encode10_pipe pcu(clk,
                                       rst_n,
                                       pu_start,
                                       NORMAL,
                                       pu_busy,
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (in_valid && !in_ready && !zeroize_busy)
                error <= 1;
            if (dec_error || nv_error || e2_error || n_error || mat_error || b_error || i_error || a_error || msg_error || pu_error || pv_error)
                error <= 1;
            if (dec_nc)
                noncanonical_seen <= 1;
            if (dec_valid)
                t_hat[{dec_poly, dec_idx}] <= dec_coeff;
            if (nv_valid) begin
                if (state == NY_WAIT)
                    y[{nv_poly, nv_idx}] <= nv_coeff;
                else
                    e1[{nv_poly, nv_idx}] <= nv_coeff;
            end
            if (e2_valid)
                e2[e2_idx] <= e2_coeff;
            if (n_rr)
            ...
            if (a_rv) begin
                if (vbranch) begin
                    if (add_second)
                        ...
                        temp[linear_d[7 : 0]] <= a_rc;
                end else
                    u[{row, linear_d[7 : 0]}] <= a_rc;
            end
            if (msg_valid)
                mu[msg_idx] <= msg_coeff;
            if (pu_valid) begin
                c[pack_word * 4] <= pu_data[7 : 0];
                c[pack_word * 4 + 1] <= pu_data[15 : 8];
                c[pack_word * 4 + 2] <= pu_data[23 : 16];
                c[pack_word * 4 + 3] <= pu_data[31 : 24];
                pack_word <= pack_word + 1'b1;
            end
            if (pv_valid) begin
                c[(240 + pack_word) * 4] <= pv_data[7 : 0];
                c[(240 + pack_word) * 4 + 1] <= pv_data[15 : 8];
                c[(240 + pack_word) * 4 + 2] <= pv_data[23 : 16];
                c[(240 + pack_word) * 4 + 3] <= pv_data[31 : 24];
                pack_word <= pack_word + 1'b1;
            end
            ...
            if (zeroize_req && !zeroize_busy) begin
                ...
                case (state)
                    ...
                    INPUT:
                        if (in_valid) begin
                            if (in_keep != 4'hf || in_last != (word_count == 311)) begin
                                error <= 1;
                                busy <= 0;
                                state <= IDLE;
                            end else begin
                                if (word_count < 296) begin
                                    ek[word_count * 4] <= in_data[7 : 0];
                                    ek[word_count * 4 + 1] <= in_data[15 : 8];
                                    ek[word_count * 4 + 2] <= in_data[23 : 16];
                                    ek[word_count * 4 + 3] <= in_data[31 : 24];
                                end else if (word_count < 304) begin
                                    m[(word_count - 296) * 4] <= in_data[7 : 0];
                                    m[(word_count - 296) * 4 + 1] <= in_data[15 : 8];
                                    m[(word_count - 296) * 4 + 2] <= in_data[23 : 16];
                                    m[(word_count - 296) * 4 + 3] <= in_data[31 : 24];
                                end else begin
                                    r[(word_count - 304) * 4] <= in_data[7 : 0];
                                    r[(word_count - 304) * 4 + 1] <= in_data[15 : 8];
                                    r[(word_count - 304) * 4 + 2] <= in_data[23 : 16];
                                    r[(word_count - 304) * 4 + 3] <= in_data[31 : 24];
                                end
                                if (word_count == 311) begin
                                    for (j = 0; j < 32; j = j + 1)
                                        rho[j] <= ek[1152 + j];
                                    state <= DEC_START;
                                end else
                    ...
                    NE_WAIT:
                        if (nv_done)
                            state <= E2_START;
                    E2_START: begin
                        noise_operations <= noise_operations + 1;
                        state <= E2_WAIT;
                    end
                    E2_WAIT:
                        if (e2_done) begin
                    ...
                    A_START: begin
                        poly_add_operations <= poly_add_operations + 1;
                        state <= A_WAIT;
                    end
                    A_WAIT:
                        if (a_done) begin
