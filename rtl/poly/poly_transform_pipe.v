 ...
 * Submodules: intt_core_pipe, ntt_core_pipe, poly_workspace.
...
module poly_transform_pipe
    ...
    reg [3 : 0] state;
    reg [7 : 0] transfer_idx, load_idx_d, result_idx_d;
    reg result_available;
    reg child_zeroize_req;
    reg [2 : 0] child_done_seen;
    wire [2 : 0] child_zeroize_busy, child_zeroize_done;
    wire in_complete, out_complete;
    wire [1 : 0] in_domain, out_domain, in_owner, out_owner;
    wire in_error, out_error;
    wire ws_rd_req = (state == LOAD_READ);
    wire ws_rd_valid;
    wire [11 : 0] ws_rd_coeff;
    wire core_start = (state == START_CORE);
    wire core_busy, core_done, core_error, core_preload_ready;
    wire core_result_req = (state == READ_CORE) || (state == READ_DRAIN);
    wire core_result_valid;
    wire [11 : 0] core_result_data;
    ...
    poly_workspace ws_in(.clk(clk),
                         ...
                         .result_valid(),
                         .result_coeff(),
                         .int_rd_req(ws_rd_req),
                         .int_rd_idx(transfer_idx),
                         .int_rd_valid(ws_rd_valid),
                         .int_rd_coeff(ws_rd_coeff),
    ...
    poly_workspace ws_out(.clk(clk),
                          ...
                          .int_pair_rd0(),
                          .int_pair_rd1(),
                          .int_wr_en(core_result_valid),
                          .int_wr_idx(result_idx_d),
                          .int_wr_coeff(core_result_data),
                          .int_pair_wr_en(1'b0),
                          .int_pair_wr_idx(7'b0),
                          .int_pair_wr0(12'b0),
                          .int_pair_wr1(12'b0),
    ...
    generate
        if (IS_INVERSE) begin : intt_gen
            intt_core_pipe core(.clk(clk),
                                .rst_n(rst_n),
                                .start(core_start),
                                .busy(core_busy),
                                .done(core_done),
                                .error(core_error),
                                .preload_en(ws_rd_valid),
                                .preload_idx(load_idx_d),
                                .preload_coeff(ws_rd_coeff),
                                .preload_ready(core_preload_ready),
                                .result_rd_req(core_result_req),
                                .result_rd_idx(transfer_idx),
                                .result_rd_valid(core_result_valid),
                                .result_rd_data(core_result_data),
                                .zeroize_req(child_zeroize_req),
                                .zeroize_busy(child_zeroize_busy[2]),
                                .zeroize_done(child_zeroize_done[2]));
        end else begin : ntt_gen
            ntt_core_pipe core(.clk(clk),
                               .rst_n(rst_n),
                               .start(core_start),
                               .busy(core_busy),
                               .done(core_done),
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (ws_rd_req)
                load_idx_d <= transfer_idx;
            if (core_result_req)
                result_idx_d <= transfer_idx;
            if (in_error || out_error || core_error)
                error <= 1;
            if (start && !start_ok)
                error <= 1;
            ...
            if (result_release && !busy)
                result_available <= 0;
            if (ws_rd_valid && !core_preload_ready)
                error <= 1;
            case (state)
                IDLE:
                    if (accepted) begin
                ...
                READ_CORE:
                    if (transfer_idx == 255)
                        ...
                        transfer_idx <= transfer_idx + 1'b1;
                READ_DRAIN:
                    if (core_result_valid && result_idx_d == 255)
                        state <= PUBLISH;
                PUBLISH:
                    state <= DONE;
                DONE: begin
