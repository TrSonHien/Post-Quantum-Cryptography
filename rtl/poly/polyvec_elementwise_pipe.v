...
/*
 * Module: polyvec_elementwise_pipe
...
module polyvec_elementwise_pipe
    ...
    reg [4 : 0] state;
    reg [1 : 0] elem;
    reg [7 : 0] idx, idx_d, child_idx_d;
    reg result_available;
    reg child_zeroize_req;
    reg [3 : 0] child_done_seen;
    wire [3 : 0] child_zeroize_busy, child_zeroize_done;
    wire [2 : 0] ac, bc, oc;
    wire avc, bvc, ovc, adc, bdc, odc;
    wire [1 : 0] ad, bd, od;
    wire ae, be, oe;
    ...
    wire child_load_begin = (state == BEGIN_A) || (state == BEGIN_B);
    wire child_load_operand = (state == BEGIN_B);
    wire child_start = (state == START_CHILD);
    wire child_busy, child_done, child_error, child_load_ready;
    wire child_result_req = (state == READ_CHILD) || (state == DRAIN_CHILD);
    wire child_result_valid;
    wire [11 : 0] child_result_coeff;
    wire [1 : 0] child_result_domain;
    wire child_result_complete;
    wire child_release = (state == RELEASE_CHILD);
    wire output_write = child_result_valid;
    wire output_publish = (state == PUBLISH);
    wire source_release = (state == PUBLISH);
    wire [1 : 0] target_domain = (OP == 3) ? 2'b10 : (OP == 4) ? 2'b01
                                                               : ad;
    assign load_ready = !busy && !zeroize_busy;
    assign result_domain = result_available ? od : 0;
    assign result_complete = result_available && ovc;
    ...
    polyvec_workspace vb(.clk(clk),
                         ...
                         .load_coeff(load_coeff),
                         .load_ready(),
                         .acquire_internal(accepted && BINARY),
                         .init_internal(1'b0),
                         .init_domain(2'b00),
                         .publish_result(1'b0),
                         .release_internal(source_release && BINARY),
                         .poly_complete(bc),
                         .vector_complete(bvc),
    ...
    polyvec_workspace vo(.clk(clk),
                         .rst_n(rst_n),
                         .load_begin(1'b0),
                         .load_poly_idx(2'b00),
                         .load_domain(2'b00),
                         .load_we(1'b0),
                         .load_poly_idx_we(2'b00),
                         .load_idx(8'b0),
                         .load_coeff(12'b0),
                         .load_ready(),
                         .acquire_internal(1'b0),
                         .init_internal(accepted),
                         .init_domain(target_domain),
                         .publish_result(output_publish),
                         .release_internal(1'b0),
                         .poly_complete(oc),
                         .vector_complete(ovc),
                         .domain_consistent(odc),
                         .vector_domain(od),
                         .error(oe),
                         .result_req(result_req && result_available),
                         .result_poly_idx(result_poly_idx),
                         .result_idx(result_idx),
                         .result_valid(result_valid),
                         .result_coeff(result_coeff),
                         .int_rd_req(1'b0),
                         .int_rd_poly_idx(2'b00),
                         .int_rd_idx(8'b0),
                         .int_rd_valid(),
                         .int_rd_coeff(),
                         .int_wr_en(output_write),
                         .int_wr_poly_idx(elem),
                         .int_wr_idx(child_idx_d),
                         .int_wr_coeff(child_result_coeff),
                         .zeroize_req(child_zeroize_req),
                         .zeroize_busy(child_zeroize_busy[2]),
                         .zeroize_done(child_zeroize_done[2]));
    generate
        if (OP == 0) begin : ga
            poly_add_pipe child(.clk(clk),
                                .rst_n(rst_n),
                                .load_begin(child_load_begin),
                                .load_operand(child_load_operand),
                                .load_domain(ad),
                                .a_load_we(aird),
                                .a_load_idx(idx_d),
                                .a_load_coeff(acoeff),
                                ...
                                .b_load_idx(idx_d),
                                .b_load_coeff(bcoeff),
                                .load_ready(child_load_ready),
                                .start(child_start),
                                .busy(child_busy),
                                .done(child_done),
                                .error(child_error),
                                .result_req(child_result_req),
                                .result_idx(idx),
                                .result_valid(child_result_valid),
                                .result_coeff(child_result_coeff),
                                .result_domain(child_result_domain),
                                .result_complete(child_result_complete),
                                .result_release(child_release),
                                .zeroize_req(child_zeroize_req),
                                .zeroize_busy(child_zeroize_busy[3]),
                                .zeroize_done(child_zeroize_done[3]));
        end else if (OP == 1) begin : gs
            poly_sub_pipe child(.clk(clk),
                                .rst_n(rst_n),
                                .load_begin(child_load_begin),
                                .load_operand(child_load_operand),
                                .load_domain(ad),
                                ...
                                .zeroize_busy(child_zeroize_busy[3]),
                                .zeroize_done(child_zeroize_done[3]));
        end else if (OP == 2) begin : gr
            poly_reduce_pipe child(.clk(clk),
                                   .rst_n(rst_n),
                                   .load_begin(child_load_begin),
                                   .load_domain(ad),
                                   .load_we(aird),
                                   .load_idx(idx_d),
                                   .load_coeff({20'd0, acoeff}),
                                   .load_ready(child_load_ready),
                                   .start(child_start),
                                   .busy(child_busy),
                                   .done(child_done),
                                   ...
                                   .zeroize_busy(child_zeroize_busy[3]),
                                   .zeroize_done(child_zeroize_done[3]));
        end else if (OP == 3) begin : gn
            poly_ntt_pipe child(.clk(clk),
                                .rst_n(rst_n),
                                .load_begin(child_load_begin),
                                .load_domain(ad),
                                .load_we(aird),
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                DRAIN_A:
                    if (aird && idx_d == 255)
                        state <= BINARY ? BEGIN_B : START_CHILD;
                BEGIN_B: begin
                    idx <= 0;
                    state <= READ_B;
                end
                ...
                READ_CHILD:
                    if (idx == 255)
                        ...
                        idx <= idx + 1'b1;
                DRAIN_CHILD:
                    if (child_result_valid && child_idx_d == 255)
                        state <= RELEASE_CHILD;
                RELEASE_CHILD:
                    state <= NEXT;
                NEXT:
