...
module polyvec_elementwise_pipe #(parameter OP=0)(
 ...
 reg child_zeroize_req;reg[3:0]child_done_seen;wire[3:0]child_zeroize_busy,child_zeroize_done;
 ...
 else begin:gi poly_intt_pipe child(.clk(clk),.rst_n(rst_n),.load_begin(child_load_begin),.load_domain(ad),.load_we(aird),.load_idx(idx_d),.load_coeff(acoeff),.load_ready(child_load_ready),.start(child_start),.busy(child_busy),.done(child_done),.error(child_error),.result_req(child_result_req),.result_idx(idx),.result_valid(child_result_valid),.result_coeff(child_result_coeff),.result_domain(child_result_domain),.result_complete(child_result_complete),.result_release(child_release),.zeroize_req(child_zeroize_req),.zeroize_busy(child_zeroize_busy[3]),.zeroize_done(child_zeroize_done[3]));end endgenerate
 always@(posedge clk)begin child_zeroize_req<=0;zeroize_done<=0;
  if(!rst_n)begin state<=IDLE;busy<=0;done<=0;error<=0;elem<=0;idx<=0;idx_d<=0;child_idx_d<=0;result_available<=0;zeroize_busy<=0;zeroize_done<=0;child_zeroize_req<=0;child_done_seen<=0;end
  else if((zeroize_req===1'b1)&&!zeroize_busy)begin state<=IDLE;busy<=0;done<=0;error<=0;elem<=0;idx<=0;idx_d<=0;child_idx_d<=0;result_available<=0;zeroize_busy<=1;child_zeroize_req<=1;child_done_seen<=0;end
  else if(zeroize_busy)begin state<=IDLE;busy<=0;done<=0;error<=0;elem<=0;idx<=0;idx_d<=0;child_idx_d<=0;result_available<=0;child_zeroize_req<=1;child_done_seen<=child_done_seen|child_zeroize_done;if(&(child_done_seen|child_zeroize_done))begin zeroize_busy<=0;zeroize_done<=1;child_zeroize_req<=0;child_done_seen<=0;end end
  else begin done<=0;if(ar||br)idx_d<=idx;if(child_result_req)child_idx_d<=idx;
