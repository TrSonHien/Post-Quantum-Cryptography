 ...
 * Submodules: basecase_mul_pipe, fixed_latency_delay, mod_mul, poly_workspace, zetas_rom.
...
module poly_basemul_pipe
    (
        ...
        input wire a_load_we,
        input wire [7 : 0] a_load_idx,
        input wire [11 : 0] a_load_coeff,
        input wire b_load_we,
        input wire [7 : 0] b_load_idx,
        input wire [11 : 0] b_load_coeff,
    ...
    localparam [1 : 0] INVALID = 0, NTT = 2;
    ...
    wire ac, bc, rc;
    wire [1 : 0] ad, bd, rd, ao, bo, ro;
    wire ae, be, re;
    wire pair_req;
    wire av, bv;
    wire [11 : 0] a0, a1, b0, b1;
    wire [6 : 0] zaddr = 7'd64 + issue_pair[6 : 1];
    wire zneg = issue_pair[0];
    wire [11 : 0] zeta_mont, zeta_signed;
    wire [11 : 0] gamma_math;
    wire bc_valid;
    wire [11 : 0] c0, c1;
    wire meta_valid;
    wire [6 : 0] write_pair;
    ...
    wire publish;
    wire source_release;
    assign pair_req = (state == ISSUE);
    assign accepted = start && start_ok;
    assign publish = (state == PUBLISH);
    assign source_release = (state == PUBLISH);
    assign load_ready = !busy && !zeroize_busy;
    assign result_domain = result_available ? rd : INVALID;
    assign result_complete = result_available && rc;
    zetas_rom zr(.inverse(1'b0),
                 .addr(zaddr),
                 .zeta(zeta_mont));
    ...
    mod_mul gamma_convert(.a(zeta_signed),
                          .b(12'd1),
                          .c(gamma_math));
    poly_workspace wa(.clk(clk),
                      .rst_n(rst_n),
                      .load_begin(load_begin && !load_operand),
                      .load_domain(load_domain),
                      .load_we(a_load_we),
                      .load_idx(a_load_idx),
                      .load_coeff(a_load_coeff),
                      .load_ready(),
                      ...
                      .int_rd_valid(),
                      .int_rd_coeff(),
                      .int_pair_rd_req(pair_req),
                      .int_pair_rd_idx(issue_pair),
                      .int_pair_rd_valid(av),
                      .int_pair_rd0(a0),
                      .int_pair_rd1(a1),
                      .int_wr_en(1'b0),
                      .int_wr_idx(8'b0),
    ...
    poly_workspace wb(.clk(clk),
                      .rst_n(rst_n),
                      .load_begin(load_begin && load_operand),
                      .load_domain(load_domain),
                      .load_we(b_load_we),
                      .load_idx(b_load_idx),
                      .load_coeff(b_load_coeff),
                      .load_ready(),
                      .acquire_internal(accepted),
                      .init_internal(1'b0),
                      .init_domain(2'b0),
    ...
    poly_workspace wr(.clk(clk),
                      ...
                      .int_wr_idx(8'b0),
                      .int_wr_coeff(12'b0),
                      .int_pair_wr_en(bc_valid && meta_valid),
                      .int_pair_wr_idx(write_pair),
                      .int_pair_wr0(c0),
                      .int_pair_wr1(c1),
                      .zeroize_req(child_zeroize_req),
                      .zeroize_busy(child_zeroize_busy[2]),
                      .zeroize_done(child_zeroize_done[2]));
    basecase_mul_pipe bm(.clk(clk),
                         .rst_n(rst_n),
                         .in_valid(av && bv && !zeroize_busy),
                         .a0(a0),
                         .a1(a1),
                         .b0(b0),
                         .b1(b1),
    ...
    fixed_latency_delay #(.PAYLOAD_WIDTH(1),
                          .METADATA_WIDTH(7),
                          .LATENCY(BASECASE_LATENCY))
        md(.clk(clk),
           .rst_n(rst_n),
           .in_valid(av && bv && !zeroize_busy),
           .in_payload(1'b0),
           .in_metadata(meta_pair_d),
           .out_valid(meta_valid),
           .out_payload(unused),
           .out_metadata(write_pair),
           .zeroize_req(child_zeroize_req),
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (pair_req) begin
                gamma_d <= gamma_math;
                meta_pair_d <= issue_pair;
            end
            if (ae || be || re)
                error <= 1;
            if (start && !start_ok)
                error <= 1;
            if (busy && (load_begin || a_load_we || b_load_we || result_req || result_release))
                error <= 1;
            if (result_req && !result_available)
                error <= 1;
            if (result_release && !busy)
                result_available <= 0;
            if (bc_valid != meta_valid) begin
                error <= 1;
            end
            case (state)
                IDLE:
                ...
                DRAIN:
                    if (bc_valid && meta_valid && write_pair == 127)
                        state <= PUBLISH;
                PUBLISH:
                    state <= DONE;
                DONE: begin
