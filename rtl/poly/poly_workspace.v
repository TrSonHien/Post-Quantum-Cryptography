        ...
        if (!rst_n) begin
            ...
            if (load_we) begin
                if (owner != EXTERNAL_LOAD || load_coeff >= 12'd3329) begin
                    ...
                    if (load_idx[0]) bank_odd[load_idx[7:1]] <= load_coeff;
                    else bank_even[load_idx[7:1]] <= load_coeff;
            ...
            if (result_req) begin
                if (owner != EXTERNAL_RESULT || !complete) begin
                    ...
                    result_valid <= 1'b1;
                    result_coeff <= result_idx[0] ? bank_odd[result_idx[7:1]] :
                                                  bank_even[result_idx[7:1]];
                end
            end
            ...
            if (int_rd_req) begin
                if (owner != INTERNAL_OPERATION) begin
                    ...
                    int_rd_valid <= 1'b1;
                    int_rd_coeff <= int_rd_idx[0] ? bank_odd[int_rd_idx[7:1]] :
                                                   bank_even[int_rd_idx[7:1]];
                end
            end
            ...
            if (int_wr_en) begin
                if (owner != INTERNAL_OPERATION || int_wr_coeff >= 12'd3329) begin
                    ...
                    if (int_wr_idx[0]) bank_odd[int_wr_idx[7:1]] <= int_wr_coeff;
                    else bank_even[int_wr_idx[7:1]] <= int_wr_coeff;
