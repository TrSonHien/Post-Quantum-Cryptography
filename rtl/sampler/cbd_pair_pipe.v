...
module cbd_pair_pipe
    ...
    function automatic[2 : 0] pc3
        (input [2 : 0] v);
        begin
            pc3 = {2'd0, v[0]} + {2'd0, v[1]} + {2'd0, v[2]};
        end
    endfunction
    reg [2 : 0] x0, y0, x1, y1;
    ...
    always @* begin
        if (eta == 2) begin
            x0 = pc3({1'b0, in_bits[1 : 0]});
            y0 = pc3({1'b0, in_bits[3 : 2]});
            x1 = pc3({1'b0, in_bits[5 : 4]});
            y1 = pc3({1'b0, in_bits[7 : 6]});
        end else begin
            x0 = pc3(in_bits[2 : 0]);
            y0 = pc3(in_bits[5 : 3]);
            x1 = pc3(in_bits[8 : 6]);
        ...
        if (!rst_n) begin
            ...
            if (in_valid) begin
                if (eta != 2 && eta != 3)
                    ...
                    coeff0 <= (x0 >= y0) ? (x0 - y0) : 3329 - (y0 - x0);
                    coeff1 <= (x1 >= y1) ? (x1 - y1) : 3329 - (y1 - x1);
                end
            end
        end
    end
