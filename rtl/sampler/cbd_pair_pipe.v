...
module cbd_pair_pipe(
 ...
 always@*begin
  ...
  always@(posedge clk)begin if(!rst_n)begin out_valid<=0;coeff0<=0;coeff1<=0;error<=0;end else if(in_ready)begin out_valid<=in_valid;if(in_valid)begin if(eta!=2&&eta!=3)error<=1;else begin coeff0<=(x0>=y0)?(x0-y0):3329-(y0-x0);coeff1<=(x1>=y1)?(x1-y1):3329-(y1-x1);end end end end
endmodule
