 ...
 * Submodules: mlkem_prf, sample_poly_cbd_pipe.
...
module mlkem_noise_sampler
    ...
    reg child_start, child_zeroize_req, prf_zeroized, cbd_zeroized;
    wire pr, po, pbusy, pdone, perr;
    wire [31 : 0] pd;
    wire [3 : 0] pk;
    wire pl;
    wire cbusy, cdone, cerr, ci;
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (czdone)
                cbd_zeroized <= 1;
            if ((prf_zeroized || pzdone) && (cbd_zeroized || czdone)) begin
                child_zeroize_req <= 0;
                prf_zeroized <= 0;
                cbd_zeroized <= 0;
                zeroize_busy <= 0;
