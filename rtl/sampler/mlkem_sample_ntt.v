 ...
 * Submodules: mlkem_xof, sample_ntt_parser.
...
module mlkem_sample_ntt
    (
        ...
        output wire [31 : 0] groups_requested,
        output wire [31 : 0] candidates_accepted,
        output wire [31 : 0] candidates_rejected,
        input wire zeroize_req,
        output reg zeroize_busy,
        output reg zeroize_done);
    reg child_start, req_pending, child_zeroize_req, xof_zeroized, parser_zeroized;
    wire xi_ready, xreq_ready, xov, xlast, xctx, xbusy, xdone, xerr;
    wire [31 : 0] xdata;
    wire [3 : 0] xkeep;
    wire pbusy, pdone, perr, pready;
    wire [31 : 0] gexam;
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (pzdone)
                parser_zeroized <= 1;
            if ((xof_zeroized || xzdone) && (parser_zeroized || pzdone)) begin
                child_zeroize_req <= 0;
                xof_zeroized <= 0;
                parser_zeroized <= 0;
                zeroize_busy <= 0;
            ...
            if (issue_req && xreq_ready)
                req_pending <= 1;
            if (xov && pready) begin
                if (xkeep != 4'b0111 || !xlast) begin
                    error <= 1;
                    busy <= 0;
                end
                req_pending <= 0;
