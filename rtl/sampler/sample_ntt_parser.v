...
module sample_ntt_parser(
 ...
 assign in_ready=busy&&(qcount==0);assign out_valid=busy&&(qcount!=0);assign out_coeff=q0;assign out_index=count[7:0];assign out_domain=2'b10;
 always@(posedge clk)begin done<=0;if(!rst_n)begin busy<=0;error<=0;q0<=0;q1<=0;qcount<=0;count<=0;groups_examined<=0;candidates_accepted<=0;candidates_rejected<=0;end
