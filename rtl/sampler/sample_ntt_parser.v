...
module sample_ntt_parser
    (
        ...
        output reg [31 : 0] groups_examined,
    ...
    reg [1 : 0] qcount;
    reg [8 : 0] count;
    wire [11 : 0] d1 = {in_data[11 : 8], in_data[7 : 0]};
    wire [11 : 0] d2 = {in_data[23 : 16], in_data[15 : 12]};
    wire a1 = d1 < 3329;
    wire a2 = d2 < 3329;
    assign in_ready = !zeroize_busy && !(zeroize_req === 1'b1) && busy && (qcount == 0);
    assign out_valid = !zeroize_busy && busy && (qcount != 0);
    assign out_coeff = zeroize_busy ? 0 : q0;
    assign out_index = zeroize_busy ? 0 : count[7 : 0];
    assign out_domain = 2'b10;
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (in_valid && in_ready) begin
                groups_examined <= groups_examined + 1;
                candidates_rejected <= candidates_rejected + (!a1) + (!a2);
                if (a1) begin
                    q0 <= d1;
                    if (a2 && count < 255) begin
                        q1 <= d2;
                        qcount <= 2;
                        candidates_accepted <= candidates_accepted + 2;
                    end else begin
                        qcount <= 1;
                        candidates_accepted <= candidates_accepted + 1;
                        if (a2 && count == 255)
