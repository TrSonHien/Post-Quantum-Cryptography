...
module poly_compress_encode_pipe #(parameter integer D=10)(
 ...
 wire child_error;
 byte_encode_poly_pipe #(.D(D))u(clk,rst_n,start,busy,done,child_error,in_valid,in_ready,exact_compress(in_coeff),out_valid,out_ready,out_data,out_keep,out_last);
 assign error=child_error | (start && input_domain!=2'b01) | (in_valid&&in_ready&&in_coeff>=3329);
endmodule
