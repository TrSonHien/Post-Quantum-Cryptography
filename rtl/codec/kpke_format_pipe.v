...
module kpke_format_pipe #(parameter integer WORDS=296,parameter integer SPLIT_WORDS=288)(
 ...
 reg[9:0]count;
 assign in_ready=!zeroize_busy&&!(zeroize_req===1'b1)&&busy&&(!out_valid||out_ready);
 always@(posedge clk)begin done<=0;zeroize_done<=0;if(!rst_n)begin busy<=0;error<=0;out_valid<=0;count<=0;out_data<=0;out_keep<=0;out_last<=0;segment<=0;zeroize_busy<=0;zeroize_done<=0;end
  else if((zeroize_req===1'b1)&&!zeroize_busy)begin busy<=0;error<=0;out_valid<=0;count<=0;out_data<=0;out_keep<=0;out_last<=0;segment<=0;zeroize_busy<=1;end
  else if(zeroize_busy)begin busy<=0;error<=0;out_valid<=0;count<=0;out_data<=0;out_keep<=0;out_last<=0;segment<=0;zeroize_busy<=0;zeroize_done<=1;end
