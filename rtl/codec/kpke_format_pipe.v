 ...
 */
module kpke_format_pipe
    #(parameter integer WORDS = 296,
      parameter integer SPLIT_WORDS = 288)
    (
        input wire clk,
        input wire rst_n,
        input wire start,
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (in_valid && in_ready) begin
                if (in_keep != 4'hf || in_last != (count == WORDS - 1)) begin
                    error <= 1;
                    busy <= 0;
                    out_valid <= 0;
                end else begin
                    ...
                    out_data <= in_data;
                    out_keep <= 4'hf;
                    out_last <= (count == WORDS - 1);
                    segment <= (count >= SPLIT_WORDS);
                    count <= count + 1;
                end
            end
        end
