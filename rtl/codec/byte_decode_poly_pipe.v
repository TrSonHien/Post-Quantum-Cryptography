        ...
        if (!rst_n) begin
            ...
            if (!ovtmp && ctmp >= D && values_out < 256) begin
                if (D == 1)
                    ...
                    rawtmp = {8'd0, rtmp[3 : 0]};
                else if (D == 10)
                    rawtmp = {2'd0, rtmp[9 : 0]};
                else
                    rawtmp = rtmp[11 : 0];
                if (D == 12 && rawtmp >= 3329) begin
                    out_value <= rawtmp - 3329;
                    noncanonical_seen <= 1'b1;
                end else
                    out_value <= rawtmp;
                out_index <= values_out[7 : 0];
                values_out <= values_out + 1'b1;
                rtmp = rtmp >> D;
                ctmp = ctmp - D;
                ovtmp = 1;
            end
