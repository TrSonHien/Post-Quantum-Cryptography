 ...
 * Submodules: mlkem_keygen_internal.
...
module mlkem_keygen
    ...
    localparam IDLE = 0, RREQ = 1, RDATA = 2, I_CMD = 3, I_FEED = 4, I_WAIT = 5, SCRUB = 6, SCRUB_WAIT = 7;
    reg [2 : 0] state;
    reg [7 : 0] entropy[0 : 63];
    reg [4 : 0] word_count;
    reg [6 : 0] scrub_addr;
    reg explicit_scrub, child_zeroize_req, child_zeroized;
    wire ic_ready, ii_ready, io_valid, io_last, io_kind, ib, ido, ier, izb, izd;
    wire [31 : 0] io_data, icycles;
    wire [3 : 0] io_keep;
    ...
    assign rng_req_len_bytes = 64;
    assign rng_data_ready = state == RDATA && !zeroize_req;
    assign out_valid = state == I_WAIT && io_valid;
    assign out_data = io_data;
    assign out_keep = io_keep;
    assign out_last = io_last;
    assign out_kind = io_kind;
    mlkem_keygen_internal core(clk,
                               rst_n,
                               state == I_CMD,
                               ic_ready,
                               state == I_FEED,
                               ii_ready,
                               {entropy[word_count * 4 + 3], entropy[word_count * 4 + 2], entropy[word_count * 4 + 1], entropy[word_count * 4]},
                               4'hf,
                               word_count == 15,
                               io_valid,
                ...
                case (state)
                    ...
                    RDATA: begin
                        if (rng_fail) begin
                            ...
                            if (rng_keep != 4'hf || rng_last != (word_count == 15)) begin
                                error <= 1;
                                scrub_addr <= 0;
                                child_zeroize_req <= 1;
                                child_zeroized <= 0;
                                ...
                                for (lane = 0; lane < 4; lane = lane + 1)
                                    entropy[word_count * 4 + lane] <= rng_data[lane * 8 +: 8];
                                if (word_count == 15)
                                    state <= I_CMD;
                                else
                    ...
                    SCRUB: begin
                        entropy[scrub_addr] <= 0;
                        if (scrub_addr == 63) begin
                            scrub_addr <= 0;
                            state <= SCRUB_WAIT;
                        end else
