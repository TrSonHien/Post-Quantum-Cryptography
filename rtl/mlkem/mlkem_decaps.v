 ...
 * Submodules: mlkem_decaps_input_check, mlkem_decaps_internal.
...
module mlkem_decaps
    ...
    localparam IDLE = 0, LOAD = 1, CWAIT = 2, CZ = 3, CZW = 4, I_CMD = 5, I_FEED = 6, I_WAIT = 7, SCRUB = 8, SCRUB_WAIT = 9;
    reg [3 : 0] state;
    reg [7 : 0] dk[0 : 2399], c[0 : 1087];
    reg [9 : 0] word_count;
    reg [11 : 0] scrub_addr;
    reg check_pass, explicit_scrub;
    reg [1 : 0] child_zeroize_req, child_zeroized;
    integer lane, reset_i;
    wire cb, cr, cd, ce, cv, czb, czd;
    wire [5 : 0] cc;
    wire icr, iir, iov, iol, ib, id, ie, izb, izd;
    wire [31 : 0] iod, icy;
    wire [3 : 0] iok;
    wire [10 : 0] cmp;
    wire [5 : 0] sel;
    ...
    assign in_ready = state == LOAD && cr && !zeroize_req;
    assign out_valid = state == I_WAIT && iov && !zeroize_req;
    assign out_data = iod;
    assign out_keep = iok;
    assign out_last = iol;
    mlkem_decaps_input_check chk(clk,
                                 rst_n,
                                 cmd_valid && cmd_ready,
                                 cb,
                                 in_valid && state == LOAD,
    ...
    mlkem_decaps_internal core(clk,
                               rst_n,
                               state == I_CMD,
                               icr,
                               state == I_FEED,
                    ...
                    LOAD:
                        if (in_valid && in_ready) begin
                            if (!in_kind)
                                ...
                                for (lane = 0; lane < 4; lane = lane + 1)
                                    c[(word_count - 600) * 4 + lane] <= in_data[lane * 8 +: 8];
                            if (word_count == 871)
                                state <= CWAIT;
                            else
