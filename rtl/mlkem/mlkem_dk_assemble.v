...
module mlkem_dk_assemble
    ...
    reg [11 : 0] load_count, out_count, scrub_count;
    reg [1 : 0] expected_kind;
    reg outputting, loading;
    wire [11 : 0] kind_len = (in_kind == DKPKE) ? 1152 : (in_kind == EK) ? 1184
                                                                         : 32;
    wire [11 : 0] kind_base = (in_kind == DKPKE) ? 0 : (in_kind == EK) ? 1152
                                                   : (in_kind == HASH) ? 2336
                                                                       : 2368;
    assign in_ready = loading && !zeroize_busy;
    assign out_valid = outputting;
    assign out_addr = out_count;
    assign out_last = outputting && out_count == 2399;
    assign out_data = (out_count < 1152) ? dkpke[out_count] : (out_count < 2336) ? ek[out_count - 1152]
                                                          : (out_count < 2368)   ? h[out_count - 2336]
                                                                                 : z[out_count - 2368];
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (scrub_count < 1152)
                dkpke[scrub_count] <= 0;
            else if (scrub_count < 2336)
                ek[scrub_count - 1152] <= 0;
            else if (scrub_count < 2368)
                h[scrub_count - 2336] <= 0;
            else
                z[scrub_count - 2368] <= 0;
            if (scrub_count == 2399) begin
                zeroize_busy <= 0;
                zeroize_done <= 1;
                scrub_count <= 0;
            end else
                scrub_count <= scrub_count + 1'b1;
        end else begin
            if (load_start) begin
                if (loading || outputting)
                    error <= 1;
                else begin
                    loading <= 1;
                    complete <= 0;
            ...
            if (in_valid && in_ready) begin
                if (in_kind != expected_kind || in_last != (load_count == kind_len - 1)) begin
                    ...
                    case (in_kind)
                        DKPKE:
                            dkpke[load_count] <= in_data;
                        EK:
                            ek[load_count] <= in_data;
                        HASH:
                            h[load_count] <= in_data;
                        default:
                            z[load_count] <= in_data;
                    endcase
                    if (load_count == kind_len - 1) begin
                        load_count <= 0;
                        if (expected_kind == Z) begin
                            loading <= 0;
                            complete <= 1;
                        end else
