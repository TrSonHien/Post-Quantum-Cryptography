...
module mlkem_dk_parse
    (
        ...
        output reg [1 : 0] read_kind,
        output reg [10 : 0] read_offset,
        input wire zeroize,
        output reg zeroize_busy,
        output reg zeroize_done);
    reg [7 : 0] dk[0 : 2399];
    reg [11 : 0] count, scrub_addr;
    reg loading;
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (read_req) begin
                read_valid <= 1;
                if (!complete || read_addr >= 2400) begin
                    read_data <= 0;
                    read_kind <= 0;
                    read_offset <= 0;
                    error <= 1;
                end else begin
                    read_data <= dk[read_addr];
                    if (read_addr < 1152) begin
                        read_kind <= 0;
                        read_offset <= read_addr;
                    end else if (read_addr < 2336) begin
                        read_kind <= 1;
                        read_offset <= read_addr - 1152;
                    end else if (read_addr < 2368) begin
                        read_kind <= 2;
                        read_offset <= read_addr - 2336;
                    end else begin
                        read_kind <= 3;
                        read_offset <= read_addr - 2368;
                    end
                end
            end
        end
